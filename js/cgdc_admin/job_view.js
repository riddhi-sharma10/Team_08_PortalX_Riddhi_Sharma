...
export async function render(container, app) {
    ...
    if (!jobId) {
        container.innerHTML = `
            <div class="card" style="padding: 40px; text-align: center; margin: 24px;">
                ...
                <button id="backBtnErr" class="btn-primary" style="margin-top: 24px;">Back to Companies</button>
            </div>
        `;
        document.getElementById('backBtnErr')?.addEventListener('click', () => app.navigateTo('companies'));
        return;
    }
    ...
    container.innerHTML = `
        <div style="display:flex;align-items:center;justify-content:center;min-height:400px;">
            <div style="text-align:center;color:var(--text-muted);">
                ...
                <p>Fetching real-time job details from database...</p>
            </div>
        </div>
    `;
    ...
    try {
            ...
            <div style="padding: 24px 32px;">
                <div style="display: flex; align-items: center; gap: 20px; margin-bottom: 32px;">
                    ...
                    <div>
                        ...
                        <p style="color: var(--text-muted); font-size: 1rem; margin-top: 4px;">Admin View: Position at <span style="color:var(--primary); font-weight:700;">${job.comp_name}</span></p>
                    </div>
                </div>
                ...
                <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 32px;">
                        ...
                        <div class="card" style="padding: 32px;">
                            ...
                            <div style="background: #f8fafc; border: 1px solid var(--border); border-radius: 12px; padding: 20px; display: flex; align-items: center; justify-content: space-between;">
                                <div style="display: flex; align-items: center; gap: 16px;">
                                    <div style="width: 48px; height: 48px; background: #eff6ff; color: #2563eb; border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 1.5rem;">
                                        <ion-icon name="document-outline"></ion-icon>
                                    </div>
                                    <div>
                                        <p style="margin: 0; font-weight: 700; color: var(--text-main);">Official JD Document</p>
                                        <p style="margin: 0; font-size: 0.8rem; color: var(--text-muted);">Stored in Database</p>
                                    </div>
                                </div>
                                <a href="${job.jd_pdf_url || '#'}" target="_blank" class="btn-primary" style="padding: 8px 16px; border-radius: 8px; font-size: 0.85rem; text-decoration: none;">Review Document</a>
                            </div>
                        </div>
                            ...
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 40px;">
                                <div>
                                    <h3 style="font-size: 1.1rem; font-weight: 800; margin-bottom: 20px; display: flex; align-items: center; gap: 10px;">
                                        <ion-icon name="git-branch-outline" style="color: var(--primary);"></ion-icon> Eligibility
                                    </h3>
                                    <div style="display: flex; flex-wrap: wrap; gap: 8px;">
                                        ${(job.eligible_branches && job.eligible_branches.length > 0) 
                                            ...
                                            `).join('')
                                            : '<p style="color:var(--text-muted); font-size:0.9rem;">No specific branch restrictions</p>'
                                        }
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div style="display: flex; flex-wrap: wrap; gap: 8px;">
                                        ${(job.skills && job.skills.length > 0)
                                            ...
                                            `).join('')
                                            : '<p style="color:var(--text-muted); font-size:0.9rem;">No skills listed</p>'
                                        }
                                    </div>
                                </div>
                            </div>
                    ...
                    <div style="display: flex; flex-direction: column; gap: 24px;">
                        <div class="card" style="padding: 24px;">
                            <h4 style="font-size: 0.8rem; font-weight: 800; color: var(--text-muted); text-transform: uppercase; margin-bottom: 20px;">Position Metrics</h4>
                            <div style="display: grid; gap: 20px;">
                                ...
                                <div style="display: flex; align-items: center; gap: 12px;">
                                    ...
                                    <div>
                                        ...
                                        <p style="margin: 0; font-weight: 800; color: var(--text-main);">${job.eligibility_cgpa}</p>
                                    </div>
                                </div>
                                <div style="display: flex; align-items: center; gap: 12px;">
                                    ...
                                    <div>
                                        <p style="margin: 0; font-size: 0.75rem; color: var(--text-muted); font-weight: 700;">DEADLINE</p>
                                        <p style="margin: 0; font-weight: 800; color: var(--text-main);">${deadline}</p>
                                    </div>
                                </div>
                                <div style="display: flex; align-items: center; gap: 12px;">
                                    <div style="width: 40px; height: 40px; background: #eff6ff; color: #2563eb; border-radius: 10px; display: flex; align-items: center; justify-content: center;"><ion-icon name="stats-chart-outline"></ion-icon></div>
                                    <div>
                                        <p style="margin: 0; font-size: 0.75rem; color: var(--text-muted); font-weight: 700;">STATUS</p>
                                        <p style="margin: 0; font-weight: 800; color: ${job.status === 'open' ? '#059669' : '#dc2626'}; text-transform: uppercase;">${job.status}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="card" style="padding: 24px; background: #f8fafc;">
                            <h4 style="font-size: 0.8rem; font-weight: 800; color: var(--text-muted); text-transform: uppercase; margin-bottom: 12px;">Company Link</h4>
                            ...
                            <button id="viewCompBtn" style="width: 100%; margin-top: 16px; background: white; border: 1px solid var(--border); color: var(--primary); padding: 10px; border-radius: 8px; font-weight: 700; cursor: pointer;">Go to Company Profile</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        container.innerHTML = `<div class="card" style="padding:40px;text-align:center;color:red;">Error fetching job data: ${err.message}</div>`;
    }
}
