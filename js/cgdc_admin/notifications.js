    ...
    container.innerHTML = `
        <div class="admin-dashboard-shell" style="gap:24px;">
            <div class="admin-dashboard-header" style="display:flex; justify-content:space-between; align-items:center;">
                <div>
                    <h1>Notifications</h1>
                    <p>Real-time activity feed for placements, applications and system alerts.</p>
                </div>
...
async function loadNotifications() {
    ...
    try {
        ...
        if (notifs.length === 0) {
            list.innerHTML = `
                <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; padding:60px; color:#64748b; text-align:center;">
                    ...
                    <p style="margin:4px 0 0; font-size:0.9rem;">We'll notify you when something important happens.</p>
                </div>
            `;
            return;
        }
        ...
        list.innerHTML = `<div style="padding:40px; text-align:center; color:red;">Error loading notifications. Please try again.</div>`;
    }
}
...
function getNotifConfig(notif) {
    switch (notif.type) {
        case 'message':
            return { icon: 'chatbubble-ellipses', color: '#10b981' };
        case 'alert':
...
function renderNotificationItem(id, title, text, time, icon, color, isRead, isLast, type) {
    ...
    return `
        <div class="notif-item" 
             ...
             style="display:flex; gap:16px; padding:20px 24px; border-bottom:${isLast ? 'none' : '1px solid #e2e8f0'}; align-items:flex-start; cursor:pointer; transition:background 0.2s; position:relative; ${isRead ? 'opacity:0.7;' : ''}">
