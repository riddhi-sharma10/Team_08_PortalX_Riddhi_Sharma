...
To understand the core of the system, let’s look at three key tables: **`STUDENT`**, **`JOB_PROFILE`**, and **`APPLICATION`**.
The `STUDENT` table stores fundamental details like name, email, department, and CGPA. The `JOB_PROFILE` table contains job postings from companies, detailing the role, package, and required CGPA. 

These two are connected by the `APPLICATION` table, which acts as a bridge. We call this resolving a many-to-many relationship. A student can apply to many jobs, and a job can receive applications from many students. The `APPLICATION` table stores the `s_id` (Student ID) and `job_id` (Job ID) as foreign keys to connect them together, while also tracking the status and application date. 

What's important about these relationships is the **cardinality**. For example, one student can apply to many jobs (1-to-N). Each application, however, securely points to exactly one student and exactly one job. All of these relationships are properly defined with foreign keys, which means the database automatically prevents data inconsistencies. If you try to create an application for a student that doesn't exist, the database will definitely stop you.

Beyond these core elements, the system scales up to handle the entire placement pipeline. Here is the full list of all 22 tables that make up this interconnected system:
1. `CGDC_ADMIN`
...
5. `COMPANY`
6. `JOB_PROFILE`
7. `JOB_REQUIRED_SKILL`
8. `JOB_ELIGIBILITY_BRANCH`
9. `DEPARTMENT`
10. `APPLICATION`
11. `INTERVIEW`
12. `OFFER`
13. `PLACEMENT_RECORD`
14. `RESUME`
15. `RESUME_ANALYSIS_KEYWORD`
16. `RESUME_PARSED_KEYWORD`
17. `STUDENT_SKILL`
18. `COMPANY_VISIT_HISTORY`
19. `VISIT_COVERED_STREAM`
20. `NOTIFICATION`
21. `CHAT_MESSAGE`
22. `STATUS_AUDIT_LOG`"
...
There are three levels. **First Normal Form** means every piece of data must be atomic, or indivisible. For example, imagine if our `JOB_PROFILE` table had a `required_skills` column storing a comma-separated list like 'Java, Python, SQL' in a single cell. That violates 1NF because searching for specific skills becomes a nightmare! To achieve 1NF, we eliminated that column and created a separate `JOB_REQUIRED_SKILL` table where every individual skill gets its own dedicated row.

**Second Normal Form** eliminates partial dependencies. For example, imagine if our `APPLICATION` table—which is logically identified by both a Student ID and a Job ID—also stored the `student_name` and `job_title` right next to the application status. The student's name only depends on the Student ID, not the Job ID. That's a partial dependency! To achieve 2NF, we remove those details and store them exclusively in the `STUDENT` and `JOB_PROFILE` tables. 

**Third Normal Form** eliminates transitive dependencies—where a non-key column depends on another non-key column. A great example in our system is the Student's Department. If the `STUDENT` table stored `dept_id`, `dept_name`, and the `department_head_name`, then `dept_name` is depending on `dept_id`, NOT the student. To fix this and achieve 3NF, we broke it out: we created a dedicated `DEPARTMENT` table to hold the department names, and the `STUDENT` table now only holds the `dept_id` as a foreign key.

So our entire database operates safely at the 3NF level. That means we have zero data redundancy, which completely prevents update anomalies, and everything stays consistently perfectly."
...
Let's say we want to see student names with their coordinator's name. We can't get both from the `STUDENT` table alone - the coordinator information is in a separate table. So we use an INNER JOIN. We say 'take the `STUDENT` table, join it with the `PLACEMENT_COORDINATOR` table where the student's `coord_id` matches the coordinator's ID.' Now we have both pieces of information in one result.

There's also LEFT JOIN, which is different. With a left join, we get all rows from the left table even if there's no match in the right table. So if we do a left join of the `STUDENT` table and the `APPLICATION` table, we get all students, and for each student, we see their applications if they have any. Students with no applications still show up - they just have empty application fields.

Joins are incredibly powerful. We can join 4, 5, or even more tables to get a complete picture. For example, to see full placement outputs, we join 4 tables: `PLACEMENT_RECORD`, `STUDENT`, `COMPANY`, and `JOB_PROFILE`. Without joins, we'd have to get the data separately and piece it together ourselves."
...
For example, if I want to know how many students are in each department, I query the `STUDENT` table, group by the `dept_id`, and then use COUNT. The result shows: CSE has 50 students, IT has 40, ECE has 35, and so on.
...
There are different types. A **scalar subquery** returns a single value. For example, 'find the average CGPA across all rows in the `STUDENT` table, then show me students whose CGPA is above that average.' The inner query finds the average. The outer query uses that number.

An **IN subquery** returns a list. For example, 'find all `s_id`s that exist in the `PLACEMENT_RECORD` table, then show me their details from the `STUDENT` table.' The inner query finds which student IDs are placed. The outer query queries the student table to show details for those exact students.

A **correlated subquery** is more advanced. It references data from the outer query. For example, 'for each department in the `DEPARTMENT` table, show the top student by CGPA in that department.' For each department, the inner query finds the max CGPA, and the outer query shows students with that max.
...
Instead of writing a complex query every time we want student information, we created a view called `vw_student_profiles` that has all that logic built in. Under the hood, this view intelligently joins the `STUDENT`, `APPLICATION`, `INTERVIEW`, and `PLACEMENT_RECORD` tables. Then we can simply query the view like it's a regular table. It's much faster than rewriting complex queries repeatedly.

We have several views in our system. One shows complete student profiles with their application count, interview count, and placement status. Another shows placement summary by department.

Views also help with normalization. Instead of storing calculated data like average salary directly in the `COMPANY` table (which violates 3NF), we calculate it dynamically in a view using the `PLACEMENT_RECORD` and `JOB_PROFILE` tables. So the data is always current, there's no chance of it being out of sync."
...
The most important procedure we have handles offer acceptance. When a student accepts a job offer, several things need to happen across multiple tables. We need to update the `OFFER` table entirely. We need to mark the student as placed in the `STUDENT` table. We need to update the `JOB_PROFILE` table to decrease the vacancy count. And we need to explicitly insert a brand new row into the `PLACEMENT_RECORD` table. All of these must succeed together, or all must fail - we can't have a partial update.

The procedure wraps all these step updates to different tables in a transaction. If any step fails, everything rolls back. The code starts with 'START TRANSACTION', then performs all the updates to the four tables, then 'COMMIT' to save everything. Or if there's an error, 'ROLLBACK' to undo everything.

Procedures are where we implement the rules of the business directly onto the tables - like 'you can't accept two offers' or 'you must have minimum CGPA to be eligible.'"
